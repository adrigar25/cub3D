...
static void	spr_draw_pixel(t_sprite_ctx *ctx, int stripe, double tx,
                           double ys) {
  int y;
  int ty;
  int color;
...
void	sprite_draw_at(t_game *game, t_img *texture, double world_x,
  ...
  if (ctx.ty <= 0.0001)
    return ;
  scx = (int)((WIN_W / 2.0) *
              (1 + ((1.0 / (game->player.plane_x * game->player.dir_y -
                            game->player.dir_x * game->player.plane_y)) *
                    (game->player.dir_y * (world_x - game->player.pos_x) -
                     game->player.dir_x * (world_y - game->player.pos_y))) /
                       ctx.ty));
